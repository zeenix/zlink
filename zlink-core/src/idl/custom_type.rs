...
/// A custom object type definition in Varlink IDL (struct-like with named fields).
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CustomObject<'a> {
    /// The name of the custom object type.
    name: &'a str,
    /// The fields of the custom object type.
    fields: List<'a, Field<'a>>,
}

impl<'a> CustomObject<'a> {
    /// Creates a new custom object type with the given name and borrowed fields.
    ...
    /// Creates a new custom object type with the given name and owned fields.
    #[cfg(feature = "std")]
    ...
    /// Returns the name of the custom object type.
    pub fn name(&self) -> &'a str {
        self.name
    }

    /// Returns an iterator over the fields of the custom object type.
...
/// A custom enum type definition in Varlink IDL (enum-like with named variants).
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CustomEnum<'a> {
    /// The name of the custom enum type.
    name: &'a str,
    /// The variants of the custom enum type.
    variants: List<'a, &'a str>,
}

impl<'a> CustomEnum<'a> {
    /// Creates a new custom enum type with the given name and borrowed variants.
    ...
    /// Creates a new custom enum type with the given name and owned variants.
    #[cfg(feature = "std")]
    ...
    /// Returns the name of the custom enum type.
    pub fn name(&self) -> &'a str {
        self.name
    }

    /// Returns an iterator over the variants of the custom enum type.
...
mod tests {
    ...
    #[test]
    fn custom_object_serialization() {
        let field_x = Field::new("x", <f64>::TYPE_INFO);
        let field_y = Field::new("y", <f64>::TYPE_INFO);
        let fields = [&field_x, &field_y];
