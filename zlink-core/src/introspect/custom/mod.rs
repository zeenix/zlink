...
// Re-export the CustomType derive macro as Type so it's available alongside the trait
pub use zlink_macros::CustomType as Type;
...
mod tests {
    ...
    fn type_from_object() {
        ...
        let custom_obj = Object::new("Point", &fields);
        let custom_type = custom::Type::from(custom_obj);
    ...
    fn type_from_enum() {
        let custom_enum = Enum::new("Color", &[&"red", &"green", &"blue"]);
        let custom_type = custom::Type::from(custom_enum);
