...
fn parameter<'a>(input: &'a [u8]) -> IResult<&'a [u8], Parameter<'a>> {
    ...
    let (input, _) = ws(input)?;
    let (input, ty) = varlink_type(input)?;
    Ok((input, Parameter::new(name, ty)))
}

/// Parse a parameter list: (param1: type1, param2: type2).
fn parameter_list<'a>(input: &'a [u8]) -> IResult<&'a [u8], List<'a, Parameter<'a>>> {
    let (input, _) = tag("(")(input)?;
    let (input, _) = ws(input)?;
...
fn method_def<'a>(input: &'a [u8]) -> IResult<&'a [u8], Method<'a>> {
    ...
    Ok((input, Method::new(name, input_params, output_params)))
}
...
fn error_def<'a>(input: &'a [u8]) -> IResult<&'a [u8], Error<'a>> {
    ...
    Ok((input, Error::new(name, params)))
}
...
fn type_def<'a>(input: &'a [u8]) -> IResult<&'a [u8], CustomType<'a>> {
    ...
    let (input, _) = ws(input)?;
    let (input, _) = tag(")")(input)?;
    Ok((input, CustomType::new(name, List::from(fields))))
}
...
fn interface_def<'a>(input: &'a [u8]) -> IResult<&'a [u8], Interface<'a>> {
    ...
    Ok((remaining, Interface::new(name, List::from(members))))
}
...
mod tests {
    ...
    fn test_parse_composite_types() {
        ...
        match parse_type("?int").unwrap() {
            Type::Optional(optional) => {
                assert_eq!(*optional.inner(), Type::Int);
            }
        ...
        match parse_type("[]string").unwrap() {
            Type::Array(array) => {
                assert_eq!(*array.inner(), Type::String);
            }
    ...
    fn test_parse_nested_types() {
        ...
        match parse_type("?[]string").unwrap() {
            Type::Optional(optional) => match optional.inner() {
                Type::Array(array) => {
                    assert_eq!(*array.inner(), Type::String);
                }
    ...
    fn test_parse_method() {
        ...
        match parse_method(input).unwrap() {
            method => {
    ...
    fn test_parse_error() {
        ...
        match parse_error(input).unwrap() {
            error => {
    ...
    fn test_parse_custom_type() {
        ...
        match parse_custom_type(input).unwrap() {
            custom_type => {
        ...
        match parse_custom_type(input).unwrap() {
            custom_type => {
                ...
                assert_eq!(custom_type.fields.len(), 3);
            }
        }
    ...
    fn test_parse_field() {
        ...
        match parse_field(input).unwrap() {
            field => {
        ...
        match parse_field(input).unwrap() {
            field => {
                ...
                match field.ty {
                    Type::Array(array) => {
                        assert_eq!(*array.inner(), Type::Int);
                    }
                    _ => panic!("Expected array type"),
                }
            }
...
error NotFound(id: int)
        ...
        match parse_interface(input).unwrap() {
            interface => {
